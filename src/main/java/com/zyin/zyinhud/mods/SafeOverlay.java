...
public class SafeOverlay extends ZyinHUDModBase
{
    ...
    protected void RenderUnsafeMarker(Position position)
    {
        ...
        double boundingBoxMaxY = block.getBoundingBox(block.getBlockState().getBaseState(), mc.theWorld, new BlockPos(position.x, position.y, position.z)).maxY;   //almost always 1, but farmland is 0.9375
    ...
    class Position
    {
        ...
        public boolean CanMobsSpawnInBlock(int dx, int dy, int dz)
        {
            ...
            if (block.isOpaqueCube(block.getBlockState().getBaseState()))	//majority of blocks: dirt, stone, etc.
            {
                return false;
            }
        ...
        public boolean IsOpaqueBlock(int dx, int dy, int dz)
        {
            ...
            return block.isOpaqueCube(block.getBlockState().getBaseState());
        }
